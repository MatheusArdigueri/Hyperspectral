...
```{r}
names(selected_bands_wl)
```


```{r Calculating Vegetation indices}
ponder_long <- ponder %>%
  pivot_longer(
    cols = starts_with("b_"),
    names_to = "band",
  ...
  pivot_longer(cols = ca:etr,
                names_to = "variable",
                values_to = "var_val")

selected_reflectance <- ponder_long %>%
  left_join(selected_bands_wl, by = "band")
```

```{r}
sel_ref2 <- selected_reflectance %>%
  mutate(
    phys_value = case_when(
      variable == "pn"   ~ pn,
      variable == "gs"   ~ gs,
      variable == "etr"  ~ etr,
      variable == "ca"   ~ ca,
...
```{r Heatmap Spearman correlation}
library(dplyr)
library(ggplot2)

# reorder-within-facet trick (no extra packages)
ggplot(spearman_facet, aes(x = x_in_facet, y = 1, fill = rho)) +
  geom_tile() +
...
sel_top <- selected_bands_wl %>%
  group_by(variable) %>%
  slice_max(abs_coef, n = topK, with_ties = FALSE) %>%
  ungroup() %>%
  select(variable, band, wavelength) %>%
  distinct()

spearman_sel_top <- ponder_long %>%
  inner_join(sel_top, by = "band") %>%
  mutate(
    phys_value = case_when(
      variable == "pn"   ~ pn,
      variable == "gs"   ~ gs,
  ...
  filter(!is.na(phys_value)) %>%
  group_by(variable, band, wavelength) %>%
  summarise(
    rho = cor(reflectance, phys_value, method = "spearman", use = "complete.obs"),
    .groups = "drop"
  )

library(forcats)

spearman_sel_top2 <- spearman_sel_top %>%
  mutate(
    xlab = paste0(round(wavelength), " nm"),
    xlab = fct_reorder(xlab, wavelength)   # order left→right by wavelength
  )

ggplot(spearman_sel_top2, aes(x = xlab, y = variable, fill = rho)) +
  geom_tile() +
  labs(x = "Selected bands", y = NULL, fill = "Spearman ρ") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
```

```{r}
phys_vars <- c("pn","gs","etr","ca","cb","cx_c")

spearman_all <- ponder_long %>%
  pivot_longer(
    cols = all_of(phys_vars),
    names_to = "variable",
    values_to = "phys_value"
  ) %>%
  group_by(variable, band_num, wavelength) %>%
  summarise(
    rho = cor(reflectance, phys_value, method = "spearman", use = "complete.obs"),
    .groups = "drop"
  )
...
```{r}
ggplot(spearman_all, aes(x = wavelength, y = variable, fill = rho)) +
  geom_tile() +
  labs(x = "Wavelength (nm)", y = NULL, fill = "Spearman ρ") +
  theme_minimal()
```
